            ...
            if (gray_valid) begin
                ...
                if ((gray_x >= 2) && (gray_y >= 2)) begin
                    case (algo_sel)
                        ...
                        2'b10: begin  // Laplacian
                            gx = $signed({5'd0, top0}) + $signed({5'd0, top1}) + $signed({5'd0, top2})
                               + $signed({5'd0, mid0})                          + $signed({5'd0, mid2})
                               + $signed({5'd0, bot0}) + $signed({5'd0, bot1}) + $signed({5'd0, bot2})
                               - ($signed({5'd0, mid1}) <<< 3);
                            gy = 12'sd0;
                            abs_gx = 13'd0;
                            abs_gy = 12'd0;
                            mag = gx[12] ? (~gx + 13'd1) : gx;
                        end

                        2'b11: begin  // Robert
                            gx = $signed({4'd0, top1}) - $signed({4'd0, mid2});
                            gy = $signed({4'd0, top2}) - $signed({4'd0, mid1});
                            abs_gx = gx[12] ? (~gx + 13'd1) : gx;
                            abs_gy = gy[11] ? (~gy + 12'd1) : gy;
                            mag = abs_gx + abs_gy;
                        end
