...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--algo",
        choices=("sobel", "prewitt", "laplacian", "robert"),
        help="edge detection algorithm selection",
    )
...
def send_requested_controls(
    ...
    if algo is not None:
        send_control_command(ser, CONTROL_ALGO, algo_values[algo])
...
def main() -> int:
    ...
    if args.control_only and not any(
        value is not None for value in (args.mode, args.threshold, args.overlay, args.algo)
    ):
        raise ValueError(
            "--control-only needs at least one of --mode, --threshold, --overlay, or --algo"
        )
    ...
    try:
        with serial.Serial(args.port, args.baud, timeout=0, write_timeout=2) as ser:
            time.sleep(0.2)
            send_requested_controls(ser, args.mode, args.threshold, args.overlay, args.algo)
            if any(value is not None for value in (args.mode, args.threshold, args.overlay, args.algo)):
