...
class CameraUartGui(tk.Tk):
    def __init__(self) -> None:
        ...
        self.control_queue: queue.Queue[tuple[str, int, str, str]] = queue.Queue()
        ...
        self.control_algo_var = tk.StringVar(value="sobel")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(control_frame, text="Algo").grid(row=0, column=5, sticky="w", padx=(16, 6), pady=4)
        self.control_algo_combo = ttk.Combobox(
            control_frame,
            textvariable=self.control_algo_var,
            width=12,
            values=("sobel", "prewitt", "laplacian", "robert"),
            state="readonly",
        )
        self.control_algo_combo.grid(row=0, column=6, sticky="w", pady=4)
        ttk.Button(control_frame, text="Send Control", command=self.send_control).grid(
            row=0, column=7, sticky="w", padx=(8, 0), pady=4
        )
    ...
    def send_control(self) -> None:
        ...
        if self.worker_thread and self.worker_thread.is_alive():
            algo = self.control_algo_var.get()
            self.control_queue.put((mode, threshold, overlay, algo))
            self._log(
                "queued control: "
                f"mode={mode} threshold={threshold} overlay={int(self.control_overlay_var.get())} algo={algo}"
            )
            return
        ...
        try:
            ...
            with serial.Serial(port, baud, timeout=0, write_timeout=2) as ser:
                time.sleep(0.1)
                send_requested_controls(ser, mode, threshold, overlay, algo)
            self._log(
                "sent control: "
                f"mode={mode} threshold={threshold} overlay={int(self.control_overlay_var.get())} algo={algo}"
            )
    ...
    def _read_config(self) -> dict:
        ...
        return {
            ...
            "control_algo": self.control_algo_var.get(),
        }
    ...
    def _send_queued_controls(self, ser) -> None:
        ...
        mode, threshold, overlay, algo = latest
        send_requested_controls(ser, mode, threshold, overlay, algo)
        self._log(f"sent queued control: mode={mode} threshold={threshold} overlay={overlay} algo={algo}")
