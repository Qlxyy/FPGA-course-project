...
static void control_write_defaults(void)
{
    ...
    Xil_Out32(CTRL_ALGO_ADDR, (u32)algo_select);
}

static void control_print_state(void)
{
    xil_printf("control: mode=%d threshold=%d overlay=%d algo=%d\r\n",
               (u32)display_mode,
               (u32)threshold_value,
               (u32)overlay_enable,
               (u32)algo_select);
...
int main(void)
{
    ...
    xil_printf("BRAM base: 0x%x, baud: %d, image: %dx%d\r\n",
               ...
               (u32)IMG_WIDTH,
               (u32)IMG_HEIGHT);
    xil_printf("control frame: a5 5a cmd value, cmd 1=mode 2=threshold 3=overlay 4=algo\r\n");
